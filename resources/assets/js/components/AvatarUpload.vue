...
import Modal from '../commponents/dashboard/Modal'
...
export default {
  ...
  data() {
    return {
      cropImage: undefined,
      dialogVisible: false,
      imageType: 'image/png,image/git,image/jpeg/,image/jpg,image/tiff'
    }
  },
  methods: {
    upload(e) {
      ...
      this.$axios.post('file/upload', formData)
        .then(res => {
          this.cropImage = res.data
          thia.dialogVisible = true
        })
    },
    succeed() {
